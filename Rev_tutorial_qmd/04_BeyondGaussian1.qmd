...
Note that we can also conduct the CMP model in `brms` using the `family = brmsfamily("com_poisson")` family, but it is still at an experimental stage and may not work well.
...
# glmmTMB ----
# Location part
emm_mu_link <- emmeans(model_1, ~ condition * sex,
...
# Scale part
emm_disp_link <- emmeans(model_1, ~ condition * sex,
...
dat_pref$pred_logmu <- predict(model_1, type = "link", re.form = NULL)
