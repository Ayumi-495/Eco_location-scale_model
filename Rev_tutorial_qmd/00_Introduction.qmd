...
## Interpreting regression coefficients on the log scale in terms of percentage change

Many of the models we use (Gaussian with log-transformed response, negative binomial, zero-one-inflated beta, Conway–Maxwell–Poisson, etc.) employ a **log link** for at least one distributional parameter. In these cases, regression coefficients ($\beta$) describe **multiplicative changes** in the parameter of interest. These can be interpreted as percentage changes after exponentiation.

### Rule of thumb

If the parameter of interest is modelled on the natural log scale, then\
$$
  \% \Delta = 100 \times \left(\exp(\beta) - 1\right)
$$\
gives the percent change in the parameter for a one-unit increase in the predictor.

### Intercept vs. coefficients

-   **Intercept:**
    -   If the response was log-transformed before fitting (e.g. Gaussian on log(Y)), exponentiating the intercept gives the **geometric mean** of Y in the reference group. This is close to the arithmetic mean when residual variance is small, but strictly speaking they differ.\
    -   If a log link was used (e.g. Poisson, negative binomial, Gaussian with log link), exponentiating the intercept gives the expected **arithmetic mean** of Y in the reference group.
-   **Coefficients:**
...
### What the percent change applies to…

-   **Log-transformed response (e.g. Gaussian on log(Y)):**\
    Coefficients are additive effects on log(Y). After exponentiation, these correspond to multiplicative changes in the original response Y. For example, a coefficient of 0.05 implies that the mean of Y is multiplied by $\exp(0.05) \approx 1.05$, i.e. about a 5% increase.

-   **Models with a log link (e.g. Poisson, negative binomial, Gaussian with log link):**\
    Coefficients directly represent multiplicative changes in the mean of Y. A coefficient of 0.05 again corresponds to about a 5% increase in the expected response.

-   **Negative binomial (log link for** $\theta$, dispersion):\
    $\% \Delta$ applies to the precision parameter $\theta$ (larger $\theta$ = weaker overdispersion).

-   **Conway–Maxwell–Poisson (**$\nu$, log link):\
    $\% \Delta$ applies to the dispersion parameter $\nu$ ($\nu > 1$ = under-dispersion, $\nu < 1$ = over-dispersion).

-   **Zero-one-inflated beta:**
    -   Location (mean, logit link) -> odds ratios (not %Δ directly).\
...
-   **Confidence/credible intervals:** Transform the endpoints. If $\beta$ ranges from L to U, then the ratio is between exp(L) and exp(U), and the percentage change is between 100 × (exp(L) − 1) and 100 × (exp(U) − 1).
