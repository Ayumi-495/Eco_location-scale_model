...
Before moving on to the examples, we provide a brief overview of how to interpret regression coefficients when the response variable or a distributional parameter is modelled on the non-natural scale. This is relevant for many of the models we will discuss in this tutorial.
The models we use (Gaussian with log-transformed response, negative binomial, zero-one-inflated beta, Conway–Maxwell–Poisson, etc.) employ a **log link** for at least one distributional parameter. In these cases, regression coefficients ($\beta$) describe **multiplicative changes** in the parameter of interest. After exponentiation, these can usually be interpreted as percentage changes, with the important exception of logit-linked parameters, which yield **odds ratios** instead.

### Rule of thumb

-   **When using a log link:**
    -   $\exp(\beta)$ gives the **multiplicative ratio** associated with a one-unit increase in the predictor.
    -   $100 \times (\exp(\beta) - 1)$ gives the **percent change** associated with a one-unit increase in the predictor.\
...
-   **Intercept:**
    -   If the response was log-transformed before fitting (e.g. Gaussian on log(Y)), exponentiating the intercept gives the **geometric mean** of Y in the reference group.\
    -   If a log link was used (e.g. Poisson, negative binomial), exponentiating the intercept gives the **arithmetic mean** of Y in the reference group.
-   **Coefficients:**
