...
## Result figures
The figure visualises results from the `glmmTMB` and `brms` models.

-   **Location part**: Points and error bars indicate the estimated `log(AdTarsus)` for each combination of treatment and sex. In `glmmTMB`, values are estimated marginal means on the log scale with 95% confidence intervals, whereas in `brms` they are posterior medians with 95% credible intervals. Only four group-level estimates (2 sexes × 2 treatments) are relevant here, so neither raw data nor fitted values are plotted (no random effects are included, so points would not scatter).

-   **Scale part**: Points and error bars show the estimated log($\sigma$), the dispersion parameter of the Gaussian distribution, for each treatment–sex cell. Because $\sigma$ is a distributional parameter rather than an observed variable, no observation-level points can be plotted; only group-level estimates with uncertainty intervals are shown.

Although raw data could be plotted for both the location and scale parts, these would not represent model-derived estimates and are therefore omitted here.
...
draws_link <- tidybayes::linpred_draws(
  brms_g1, newdata = nd, re_formula = NA, dpar = TRUE, transform = FALSE
) %>%
  transmute(
    Sex, Treatment, .draw,
