---
title: "Location–scale models in ecology: handling heterogeneity in continuous, count and proportion data"
...
-   There are a significant negative interaction between sex and treatment (`SexMale:TreatmentFed`; $\beta_{[\text{interaction}]}^{(s)}$ = -0.95, 95% CI = \[-1.66, -0.24\]). Back-transformation shows that residual variance among supplemented males was only ~19–79% of that observed in control males (point estimate: 39%). In other words, food supplementation led to markedly more uniform growth among males, reducing variation in adult tarsus length. Neither treatment nor sex alone significantly affected variance; the reduction was specific to supplemented males.
